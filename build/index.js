...
server.tool("flux_kontext_max_edit", {
    ...
    const { prompt, image_url, seed, guidance_scale = 3.5, sync_mode = false, num_images = 1, safety_tolerance = "2", output_format = "jpeg", aspect_ratio = "1:1" } = args;
    try {
        // Prepare input for FAL API
        const input = {
            prompt,
...
server.tool("flux_kontext_max_edit_async", {
    ...
    try {
        ...
        while (attempts < maxAttempts) {
            ...
            // Check if we should continue polling
            if (status.status !== "IN_QUEUE" && status.status !== "IN_PROGRESS") {
                throw new Error(`Image editing failed with status: ${status.status}`);
            }
            // Wait 5 seconds before next check
            await new Promise(resolve => setTimeout(resolve, 5000));
            attempts++;
